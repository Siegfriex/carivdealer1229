...
def process_markdown(input_file, output_file):
    ...
    while i < len(lines):
        ...
        if '|' in line and not line.strip().startswith('|---'):
            ...
            is_header_row = '| 필드명 |' in prev_line or '| 필드명' in prev_line
            
            if is_header_row and '소스(근거 문서)' in prev_line:
                # 헤더 행에서 소스(근거 문서) 컬럼 위치 찾기
                header_parts = [p.strip() for p in prev_line.split('|')]
                header_parts = [p for p in header_parts if p]
                if '소스(근거 문서)' in header_parts:
                    source_col_idx = header_parts.index('소스(근거 문서)')
                    # 현재 데이터 행 처리
                    parts = line.split('|')
                    if len(parts) > source_col_idx + 1:
                        source_value = parts[source_col_idx + 1].strip()
                        if source_value and not re.search(r'\[\^\d+\]', source_value):
                            footnote_id = get_footnote_id(f"소스(근거 문서): {source_value}")
                            parts[source_col_idx + 1] = f" {source_value} [^{footnote_id}] "
                            line = '|'.join(parts)
    ...
    if footnotes:
        # 기존 각주 섹션이 있는지 확인
        if '## 각주' not in content:
            content += '\n\n---\n\n## **각주**\n\n'
        else:
            # 기존 각주 섹션 찾기
            footnote_section_start = content.find('## 각주')
...
if __name__ == '__main__':
    ...
    print("명령어: pandoc PRD_Phase1_2025-12-31_copy2_processed.md -o PRD_Phase1_2025-12-31_copy2.docx --toc --toc-depth=3")
