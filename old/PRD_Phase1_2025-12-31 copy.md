...
**버전**: v1.1 (IA/사이트맵 표준화 보강)  
**마일스톤**: 2025-12-31  
**작성일**: 2025-12-23  
**최종 업데이트**: 2025-12-23  
**작성자**: ForwardMax 기획팀  
**검증 상태**: 수정 완료(Phase 1 기준 / IA 표준화 반영)  
...
> 참조: `SSOT_딜러플랫폼_니즈.md` Section 2, `웹앱환경분석데스크리서치.md`
...
### 8.0 사이트맵/IA 표준(표 형식) — 이미지 규칙 준수

본 문서의 사이트맵/IA는 **첨부 이미지(3종) 규칙을 표준으로 채택**한다. DOCX 변환을 염두에 두고, **표(테이블)만 사용**하여 구조를 표현한다.

#### 8.0.1 표준 열(컬럼) 정의 — (이미지 1~2 기준: 1depths/2depths/3depths/contents)

| 컬럼 | 의미 | 작성 규칙 |
|---|---|---|
| 탭(또는 영역) | 동일한 네비게이션 컨텍스트(GNB/LNB/Sidebar 기준 묶음) | 예: `딜러(대시보드)`, `바이어(대시보드)`, `공통(인증)` |
| 1depths | 최상위 메뉴(그룹) | 예: 차량 관리, 판매 관리 |
| 2depths | 화면/리스트/페이지 단위 | 예: 차량 등록, 차량 목록 |
| 3depths | 2depth의 하위 뷰/세부 단계(상세/설정/서브페이지 등) | 없으면 `-` 표기 |
| contents | 사용자가 할 수 있는 일(기능 요약) + ID 참조 | `SCR-####`, `FLOW-##`, `FUNC-##`는 **contents에 포함** |

#### 8.0.2 행(로우) 작성 규칙 — (DOCX 친화)

- **1행 = “하나의 네비게이션 경로 + 그 경로에서 제공되는 핵심 행동(기능)”**을 표현한다.
- 1/2/3depth 중 **없거나 미정인 단계는 `-`**로 표기한다(병합 셀 사용 금지).
- 한 행의 contents는 **1~2문장**으로 끝내고, 상세는 FRD 섹션(`10. FRD`)에서 확장한다.

#### 8.0.3 사이트맵 섹션 시작 규칙 — (이미지 3 기준: ‘가로 컬럼형 사이트맵’)

사이트맵 섹션(8장)은 **가로 컬럼형(3번째 이미지 스타일) 전체 개요 표**로 시작하고, 이후에 **Depth(1/2/3depths) 상세 표**를 제공한다.

### 8.1 전체 사이트맵(개요) — 가로 컬럼형(이미지 3 스타일)

> 표기 규칙: 셀 내 항목은 `<br>`로 줄바꿈하여 DOCX 변환 시에도 가독성을 유지한다.

| 공통(Common) | 딜러(Dealer) | 바이어(Buyer) |
|---|---|---|
| 로그인 `SCR-0001`<br>회원가입(딜러) `SCR-0002`<br>사업자 인증 `SCR-0003`<br>마이페이지(GNB) `-`(Phase 1 상세 미정)<br>알림(GNB) `-`(Phase 1 상세 미정)<br>설정(GNB) `-`(Phase 1 상세 미정) | 딜러 대시보드 `SCR-0100`<br>차량 관리(등록/목록/상세)<br>판매 관리(경매/일반제안/내역)<br>검차 관리(신청/결과)<br>탁송 관리(예약/내역)<br>정산 관리(내역/상세) | 바이어 대시보드 `SCR-3000`<br>차량 검색/조회(경매 목록/상세/즉시구매)<br>내 입찰/구매(입찰/구매 내역)<br>계약/결제(계약서/결제 내역) |

### 8.2 IA 상세(Depth 표) — 딜러(Dealer)

| 탭(또는 영역) | 1depths | 2depths | 3depths | contents |
|---|---|---|---|---|
| 딜러(대시보드) | 대시보드 | 딜러 대시보드 | - | `SCR-0100`: 업무 현황/진행 상태를 요약하여 제공한다. |
| 딜러(차량 관리) | 차량 관리 | 차량 등록 | - | `SCR-0200`(`FLOW-03`): 차량번호 입력 → 등록원부 OCR(`FUNC-05`)로 기본정보 자동 채움 후 검차 신청으로 연결한다. |
| 딜러(차량 관리) | 차량 관리 | 차량 목록 | - | `SCR-0101`: 내 차량 리스트를 상태( Draft/Inspecting/Active 등) 기준으로 조회한다. |
| 딜러(차량 관리) | 차량 관리 | 차량 상세/판매 방식 선택 | - | `SCR-0300`(`FLOW-03`): 판매 방식 선택 및 일반→경매 전환(`FUNC-15`)을 제공한다. |
| 딜러(판매 관리) | 판매 관리 | 경매 상세 | - | `SCR-0400`(`FLOW-03`): 경매 설정(시작가/즉시구매가) 및 진행 상태를 표시한다. |
| 딜러(판매 관리) | 판매 관리 | 일반 판매 제안 목록 | - | `SCR-0102`(`FLOW-03`): 바이어 제안을 조회하고 수락(`FUNC-16`) 처리한다. |
| 딜러(판매 관리) | 판매 관리 | 판매 내역 | - | `SCR-0103`: 판매/거래 완료 이력을 조회한다. |
| 딜러(검차 관리) | 검차 관리 | 검차 신청 | - | `SCR-0201`(`FLOW-02`): 희망 일정 입력 후 검차 신청(`FUNC-06`)을 수행한다. |
| 딜러(검차 관리) | 검차 관리 | 검차 결과 조회 | - | `SCR-0202`(`FLOW-02`): 검차 결과를 조회하며 **수정은 불가**(정책)하다. |
| 딜러(탁송 관리) | 탁송 관리 | 탁송 예약/배차 | - | `SCR-0600`(`FLOW-04`): 금전 절차 완료 후에만 탁송 신청이 가능하도록 제어한다. |
| 딜러(탁송 관리) | 탁송 관리 | 탁송 내역 | - | `SCR-0601`(`FLOW-04`): 배차/인계 진행 내역을 확인하고 인계 승인(`FUNC-22`)을 수행한다. |
| 딜러(정산 관리) | 정산 관리 | 정산 내역 | - | `SCR-0104`: 정산 리스트를 조회한다. |
| 딜러(정산 관리) | 정산 관리 | 정산 상세 | - | `SCR-0105`(`FLOW-04`): 정산 상세를 조회하고 “탁송 신청” 진입을 제공한다. |

### 8.3 IA 상세(Depth 표) — 바이어(Buyer)

| 탭(또는 영역) | 1depths | 2depths | 3depths | contents |
|---|---|---|---|---|
| 바이어(대시보드) | 대시보드 | 바이어 대시보드 | - | `SCR-3000`: 바이어 홈/대시보드의 진입점이다. |
| 바이어(차량 검색/조회) | 차량 검색/조회 | 경매 조회/입찰 | - | `SCR-0500`(`FLOW-03`): 경매 차량을 탐색/조회하고 입찰(`FUNC-18`)을 수행한다. |
| 바이어(차량 검색/조회) | 차량 검색/조회 | 경매 조회/입찰 | 즉시구매 | `SCR-0500`(`FLOW-03`): 즉시구매(`FUNC-19`)는 동일 화면의 서브 플로우(모달/확인 단계)로 제공한다. |
| 바이어(내 입찰/구매) | 내 입찰/구매 | 입찰 내역 | - | `SCR-3001`: 내 입찰 이력을 조회한다. |
| 바이어(내 입찰/구매) | 내 입찰/구매 | 구매 내역 | - | `SCR-3002`: 내 구매 이력을 조회한다. |
| 바이어(계약/결제) | 계약/결제 | 계약서 확인 | - | `SCR-3003`: 계약서를 확인한다. |
| 바이어(계약/결제) | 계약/결제 | 결제 내역 | - | `SCR-3004`: 결제 내역을 조회한다. |

### 8.4 공통 영역(인증/기본) — Depth 표

| 탭(또는 영역) | 1depths | 2depths | 3depths | contents |
|---|---|---|---|---|
| 공통(인증) | 인증 | 로그인 | - | `SCR-0001`(`FLOW-00`): 이메일/휴대폰 + 비밀번호로 로그인(`FUNC-00`)한다. |
| 공통(인증) | 인증 | 회원가입(딜러) | - | `SCR-0002`(`FLOW-01`): 기본 정보 입력 후 사업자 인증 단계로 이동한다. |
| 공통(인증) | 인증 | 사업자 인증 | - | `SCR-0003`(`FLOW-01`): 사업자등록증 업로드 → OCR/진위 확인(`FUNC-02`) 후 권한을 부여한다. |

> 주석: Phase 1에서 ‘알림/설정/마이페이지’의 상세 IA가 확정되면, 동일한 표준(8.0)을 그대로 적용해 표를 확장한다.
