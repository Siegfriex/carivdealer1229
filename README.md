...
| 구분 | 패키지 |
|------|--------|
| **dependencies** | react, react-dom, react-router-dom, firebase, @tanstack/react-query, zustand, zod, lucide-react, @google/genai |
| **devDependencies** | vite, typescript, tailwindcss, postcss, autoprefixer, vitest, @playwright/test, eslint, prettier |
...
| 구분 | 패키지 |
|------|--------|
| **dependencies** | firebase-admin, firebase-functions, express, @google/genai, @google-cloud/secret-manager, busboy, xml2js |
| **devDependencies** | typescript, @types/node, @types/express, @types/busboy, @types/xml2js |
| **Node** | 20 |
...
- **저장소**: 클론 후 위 순서대로 `npm install` → `functions` 내 `npm install` 진행.
- **무시 항목**: `.gitignore`에 `node_modules`, `dist`, `.env`, `.env.local`, `test-results/`, `playwright-report/` 등이 포함되어 있어 해당 경로는 커밋되지 않습니다.
