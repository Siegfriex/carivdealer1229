...
**플랜 B 전환**: 2026-02-08. MCP 한도로 **로컬 스크린샷 기반** 진행. 디자인 SSOT는 `FIGMASCR0208` (섹션별 폴더: §3.1_랜딩, §3.4_차량목록 등). 상세 절차는 [PLAN_B_SCREENSHOT_BASED.md](PLAN_B_SCREENSHOT_BASED.md) 참조. MCP_CYCLE_LOG 기재 시 "MCP 호출" 대신 "참조 스크린샷: FIGMASCR0208/§3.x_섹션폴더/§3.x_*" 형태로 기재.  
**라운드 1 재진행**: 라운드 1부터 다시 돌릴 때는 [PLAN_B_ROUND1_RESTART.md](PLAN_B_ROUND1_RESTART.md)를 1차 참조(사전 플랜·참조 스크린샷·에이전트별 복붙 프롬프트 포함). 태스크 부여 직전까지 해당 문서에서 검토 후 §4 프롬프트로 A/B/C 부여.
...
**라운드 1 재진행 (플랜 B)**  
라운드 1부터 다시 진행할 때: [PLAN_B_ROUND1_RESTART.md](PLAN_B_ROUND1_RESTART.md)를 열어 §3 사전 검토 체크리스트를 확인한 뒤, §4의 에이전트 A/B/C용 복사·붙여넣기 프롬프트를 각 에이전트에게 부여한다. 태스크 부여 직전까지의 준비는 해당 문서에 모두 정리되어 있음.
...
**라운드 3 — 복사·붙여넣기 프롬프트 (플랜 B, 참조 스크린샷 경로 포함)**

MCP 미사용. 아래 "참조 스크린샷" 경로의 로컬 이미지를 열어 디자인 SSOT로 사용한다. 절차는 [PLAN_B_SCREENSHOT_BASED.md](PLAN_B_SCREENSHOT_BASED.md) §4 참조.

**에이전트 A용 (라운드 3, 사이클 7, 플랜 B)**
...
- **라운드**: 3
- **섹션**: §3.4 차량 목록
- **nodeId**: 1418:15565, 1418:17357, 1418:20145
- **참조 스크린샷 (FIGMASCR0208/§3.4_차량목록/)**: §3.4_1418-15565_차량목록_등록완료.png, §3.4_1418-15565_차량목록_등록완료-1.png, -2.png, -3.png / §3.4_1418-17357_차량목록_그리드.png, §3.4_1418-17357_차량목록_그리드-1.png ~ -4.png / §3.4_1418-20145_차량목록_리스트.png, §3.4_1418-20145_차량목록_리스트-1.png
- **대표 라우트**: /vehicles (등록완료·그리드·리스트 변형)

진행 규칙 (플랜 B):
1) docs/figma/PLAN_B_SCREENSHOT_BASED.md를 읽고, FIGMASCR0208 내 참조 스크린샷 경로를 확인한다.
2) 위 참조 스크린샷을 열어 디자인을 확인한 뒤, 해당 라우트/페이지를 스크린샷에 맞게 구현 또는 수정한다(px, FIGMA_MCP_TO_CODE_CONVERSION·IA·API/ERD 참조).
3) npm run build 실행, 에러 시 수정. npm run dev 후 해당 라우트 접속해 러닝 스크린샷 캡처.
4) 참조 스크린샷 vs 러닝 스크린샷 비교 → 차이 정리 → 디버깅·리팩토링.
5) MCP_CYCLE_LOG 등 체크리스트 갱신(에이전트 A, 라운드 3, 시간 명시). 보고: 담당 사이클, 참조 스크린샷 경로, 수정 파일, 비교 요약, 남은 이슈.
```

**에이전트 B용 (라운드 3, 사이클 13, 플랜 B)**
...
- **라운드**: 3
- **섹션**: §3.5 차량 등록·상세·경매
- **nodeId**: 1418:24679, 1418:24463, 1418:21690 (24463은 스크린샷 없음 — IA·기존 코드만 참고)
- **참조 스크린샷 (FIGMASCR0208/§3.5_차량등록_상세_경매/)**: §3.5_1418-24679_거래상세.png, §3.5_1418-24679_거래상세-1.png, -2.png / §3.5_1418-21690_거래상세_경매.png, §3.5_1418-21690_거래상세_경매-1.png
- **대표 라우트**: /vehicles/:id, /vehicles/:id/auction/*

진행 규칙 (플랜 B):
1) docs/figma/PLAN_B_SCREENSHOT_BASED.md를 읽고, FIGMASCR0208 내 참조 스크린샷 경로를 확인한다.
2) 위 참조 스크린샷을 열어 디자인을 확인한 뒤, 해당 라우트/페이지를 스크린샷에 맞게 구현 또는 수정한다(px, FIGMA_MCP_TO_CODE_CONVERSION·IA·API/ERD 참조). 24463(거래상세 일반판매)은 IA 텍스트·기존 코드만 참고.
3) npm run build 실행, 에러 시 수정. npm run dev 후 해당 라우트 접속해 러닝 스크린샷 캡처.
4) 참조 스크린샷 vs 러닝 스크린샷 비교 → 차이 정리 → 디버깅·리팩토링.
5) MCP_CYCLE_LOG 등 체크리스트 갱신(에이전트 B, 라운드 3, 시간 명시). 보고: 담당 사이클, 참조 스크린샷 경로, 수정 파일, 비교 요약, 남은 이슈.
```

**에이전트 C용 (라운드 3, 사이클 17, 플랜 B)**

```
이번 1회 사이클을 진행해줘. 플랜 B: MCP 대신 로컬 스크린샷·IA를 디자인 SSOT로 사용한다.

- **에이전트 식별자**: 에이전트 C (§3.3 대시보드·§3.6 검차·§3.10 탁송·§3.11 정산 담당)
- **라운드**: 3
- **섹션**: §3.6 검차
- **nodeId**: 1425:9875, 1425:10137, 1425:10663 (해당 nodeId 스크린샷 없음 — IA·기존 코드 참고. 참고용: §3.6_1444-8198_검차신청_Step1.png)
- **참조 스크린샷 (FIGMASCR0208/§3.6_검차/)**: §3.6_1444-8198_검차신청_Step1.png (관련 참고). 9875/10137/10663은 IA_FSD_COMPLETE_VERIFICATION_20260208.md §3.6 및 기존 코드만 참고.
- **대표 라우트**: /inspections, /inspections/:id/progress 등

진행 규칙 (플랜 B):
1) docs/figma/PLAN_B_SCREENSHOT_BASED.md를 읽고, FIGMASCR0208·IA 문서를 확인한다.
2) IA §3.6 페이지 설명과 기존 검차 페이지 코드를 바탕으로 9875(검차 요청 내역 카드 뷰), 10137(검차 진행 매칭중), 10663(픽업/이동중)에 맞게 구현 또는 수정한다(px, FIGMA_MCP_TO_CODE_CONVERSION·API/ERD 참조).
3) npm run build 실행, 에러 시 수정. npm run dev 후 해당 라우트 접속해 러닝 스크린샷 캡처.
4) 가능한 참조 스크린샷·IA vs 러닝 스크린샷 비교 → 차이 정리 → 디버깅·리팩토링.
5) MCP_CYCLE_LOG 등 체크리스트 갱신(에이전트 C, 라운드 3, 시간 명시). 보고: 담당 사이클, 참조한 문서/스크린샷, 수정 파일, 비교 요약, 남은 이슈.
```

---

**복구 시**: 새 세션에서 본 문서를 먼저 읽고, 위 "다음 라운드"와 §2 라운드 진행표를 확인한 뒤, 해당 라운드의 복붙 프롬프트를 제시한다. **라운드 3부터는 플랜 B**: 위 "라운드 3 — 복사·붙여넣기 프롬프트 (플랜 B)" 또는 [PLAN_B_SCREENSHOT_BASED.md](PLAN_B_SCREENSHOT_BASED.md) §6 참조 스크린샷 목록으로 A/B/C용 프롬프트를 생성한다. (이전 라운드는 위 또는 [MCP_AGENT_CYCLE_INPUT_PROMPT.md](MCP_AGENT_CYCLE_INPUT_PROMPT.md) 치환 예시 표 사용.)
