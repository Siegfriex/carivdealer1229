...
test.describe('차량 등록 플로우', () => {
  test('차량 등록 Step 1-2 스크린샷 및 OCR 테스트', async ({ page }) => {
    ...
    // 헤더 노출 (LandingHeader 2단)
    await expect(page.getByText(/FORWARD/)).toBeVisible();
    await expect(page.getByRole('link', { name: '차량목록' })).toBeVisible();

    // ProgressSidebar 스텝 확인 (Step1 페이지는 ProgressSidebar 사용)
    await expect(page.locator('text=차량 업로드')).toBeVisible();

    // 페이지 제목 확인
    await expect(page.locator('text=차량 원부 등록')).toBeVisible();
    ...
    // 차량번호 입력
    await page.fill('input[placeholder="123가 4567"]', '33바 3333');
    await page.screenshot({
    ...
    // StepProgress 확인 (Step2 페이지는 StepProgress 사용)
    ...
    // 다음 버튼 클릭하여 등록 완료 페이지로 이동
    await page.click('text=다음 (검차 신청)');

    // 등록 완료 페이지 확인
    await page.waitForSelector('text=차량 등록이 완료되었습니다');
    await page.screenshot({
      path: 'tests/screenshots/05-vehicle-register-complete.png',
      fullPage: true,
    });
  });
});
