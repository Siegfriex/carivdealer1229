 ...
 * @see docs/figmaMCP/impl_plans/1123-20023_1123-20699_1123-13763_구현계획.md
 */
...
export const AuctionDurationPage = () => {
  ...
  const { startPrice, instantPrice } = (locationState as { startPrice?: string; instantPrice?: string }) ?? {};
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className={`flex min-w-0 ${LAYOUT_CLASSES.CONTENT_MIN_HEIGHT} ${LAYOUT_CLASSES.CONTAINER}`}>
        ...
        <main className={`flex-1 min-w-0 overflow-x-auto p-6 ${LAYOUT_CLASSES.MAIN_GNB_STEP}`} data-node-id="1123:20023">
          {/* 1123:20090 — 경매 기간 카드 971×336, 1123:20091~20097 */}
          <div
            className="bg-white rounded-card shadow-figma-card w-full max-w-[971px] min-h-[336px] p-6"
          ...
          >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div className="bg-form-field-bg rounded-[30px] min-h-[180px] p-6 flex flex-col" data-node-id="1123:20092">
                <p className="text-[22px] font-bold text-black/70 mb-4" data-node-id="1123:20094">
                  경매 시작일
                </p>
                <Input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="w-full text-[38px] font-bold text-black/30" aria-label="경매 시작일" />
              </div>
              <div className="bg-form-field-bg rounded-[30px] min-h-[180px] p-6 flex flex-col" data-node-id="1123:20093">
                <p className="text-[22px] font-bold text-black/70 mb-4" data-node-id="1123:20097">
                  경매 종료일
                </p>
                <Input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="w-full text-[38px] font-bold text-black/30" aria-label="경매 종료일" />
              </div>
            </div>
          </div>
          <div className="flex gap-4 mt-8" data-node-id="1123:13763">
